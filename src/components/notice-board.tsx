...
export function NoticeBannerBar({ onOpenModal }: { onOpenModal: () => void }) {
  return (
    <aside
      ...
      className="w-full bg-gradient-to-r from-amber-500/15 via-amber-500/10 to-cyan-500/15 border-b border-amber-500/30 px-3 py-2 text-xs relative z-50 backdrop-blur-md shadow-sm"
    >
      <div className="max-w-7xl mx-auto flex flex-wrap items-center justify-between gap-2">
        <div className="flex items-center gap-2 text-[var(--bos-text-primary)]">
          <span className="inline-flex items-center gap-1.5 px-2 py-0.5 rounded-full bg-amber-500/20 text-amber-400 font-mono text-[10px] font-bold tracking-wider uppercase border border-amber-500/30">
            <span className="w-1.5 h-1.5 rounded-full bg-amber-400 animate-ping" />
            Active Preview
          </span>
          <span className="font-medium text-[11px] sm:text-xs">
            <strong className="text-amber-300 font-semibold">NOT FINAL PRODUCT:</strong>{" "}
            This CRM instance is under continuous deployment & feature iteration.
          </span>
        </div>
        ...
        <button
          type="button"
          onClick={onOpenModal}
          className="inline-flex items-center gap-1 text-[11px] font-mono text-amber-300 hover:text-amber-200 underline decoration-amber-400/50 hover:decoration-amber-300 transition-colors cursor-pointer ml-auto sm:ml-0"
        >
          <span>Open Notice Board & Status</span>
      ...
      {isOpen && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 sm:p-6 overflow-y-auto">
          ...
          <motion.div
            ...
            className="relative w-full max-w-2xl bg-[#0e1017] border border-amber-500/40 rounded-2xl shadow-2xl overflow-hidden z-10 flex flex-col my-auto max-h-[90vh]"
          >
            {/* Top Glow bar */}
            <div className="h-1.5 w-full bg-gradient-to-r from-amber-500 via-yellow-400 to-cyan-400" />

            {/* Header Image with HUD Overlays */}
            <div className="relative w-full h-44 sm:h-56 shrink-0 bg-black overflow-hidden border-b border-white/10">
              <Image
                src="/development_notice_banner.jpg"
              ...
              <button
                type="button"
                onClick={handleDismiss}
                className="absolute top-3 right-3 p-1.5 rounded-full bg-black/60 hover:bg-black/90 text-white/70 hover:text-white border border-white/20 transition-all cursor-pointer z-20"
              ...
              <div className="absolute top-3 left-3 flex items-center gap-2 z-10">
                <span className="px-2.5 py-1 rounded-full bg-amber-500/30 text-amber-300 border border-amber-500/50 text-[10px] font-mono font-bold uppercase tracking-wider flex items-center gap-1.5 backdrop-blur-md">
                  <span className="w-2 h-2 rounded-full bg-amber-400 animate-pulse" />
                  PREVIEW BUILD
                </span>
                <span className="px-2 py-0.5 rounded-full bg-cyan-500/20 text-cyan-300 border border-cyan-500/40 text-[10px] font-mono backdrop-blur-md">
                  RENDER STAGING
                </span>
              </div>
              ...
              <div className="absolute bottom-3 left-4 right-4 z-10">
                <h3 className="text-xl sm:text-2xl font-bold text-white tracking-tight flex items-center gap-2">
                  ...
                  <span className="text-xs font-mono font-normal px-2 py-0.5 rounded bg-amber-500/20 text-amber-300 border border-amber-500/40">
                    v0.9.8_DevPreview
                  </span>
                </h3>
                <p className="text-xs text-amber-300/90 font-medium mt-0.5">
                  Notice: Under Active Construction & Continual Refinement
                </p>
              </div>
            </div>
            ...
            <div className="p-5 sm:p-6 overflow-y-auto space-y-5 text-sm text-slate-300">
              ...
              <div>
                ...
                <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs font-mono">
                  ...
                  <div className="p-2.5 rounded-lg bg-white/5 border border-white/10">
                    ...
                    <div className="text-cyan-300 font-semibold mt-0.5">Next.js 16 Fullstack</div>
                  </div>
                  <div className="p-2.5 rounded-lg bg-white/5 border border-white/10">
                    <div className="text-[10px] text-slate-400">DATABASE</div>
                    <div className="text-amber-300 font-semibold mt-0.5">Prisma 7 + SQLite</div>
                  </div>
                  <div className="p-2.5 rounded-lg bg-white/5 border border-white/10">
                    <div className="text-[10px] text-slate-400">BACKEND ENGINE</div>
                    <div className="text-purple-300 font-semibold mt-0.5">Control Plane Ready</div>
                  </div>
                </div>
              </div>
              ...
              <div>
                ...
                <div className="space-y-2">
                  <div className="p-3 rounded-lg bg-white/[0.03] border border-white/10 flex items-start gap-3">
            ...
            {/* Modal Footer */}
            <div className="p-4 sm:p-5 bg-black/50 border-t border-white/10 flex flex-col sm:flex-row items-center justify-between gap-3 shrink-0">
              <label className="flex items-center gap-2 cursor-pointer text-xs text-slate-400 select-none">
                <input
                  type="checkbox"
                  checked={dontShowAgain}
                  onChange={(e) => setDontShowAgain(e.target.checked)}
                  className="w-3.5 h-3.5 rounded border-white/20 bg-white/10 accent-amber-500 text-amber-500"
                />
                <span>Don't show automatically on this device</span>
              </label>
              ...
              <div className="flex items-center gap-2 w-full sm:w-auto">
                <button
                  type="button"
                  onClick={handleDismiss}
                  className="w-full sm:w-auto px-5 py-2 rounded-xl bg-gradient-to-r from-amber-500 to-amber-600 hover:from-amber-400 hover:to-amber-500 text-black font-semibold text-xs tracking-wide shadow-lg shadow-amber-500/20 transition-all cursor-pointer"
                >
                  ✓ Understood — Continue to Login
                </button>
              </div>
            </div>
          </motion.div>
...
export function NoticeFloatingBeacon({ onOpen }: { onOpen: () => void }) {
  return (
    <button
      type="button"
      onClick={onOpen}
      className="fixed bottom-4 right-4 z-40 inline-flex items-center gap-2 px-3 py-2 rounded-full bg-[#0e1017]/90 border border-amber-500/40 text-amber-300 shadow-xl backdrop-blur-md hover:border-amber-400 hover:bg-[#0e1017] transition-all group cursor-pointer"
...
export function DevelopmentNoticeSystem({
  ...
  useEffect(() => {
    try {
      ...
      if (!dismissed) {
        // Automatically reveal modal after brief delay on initial load

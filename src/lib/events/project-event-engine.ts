        ...
        for (const t of project.tasks) {
          if (!t.assigneeId && !t.assigneeName) {
            ...
            let matchedEmp = workspaceEmployees.find((e) => {
              ...
              const job = (e.jobTitle || "").toUpperCase();
              if (layer === "DATABASE" && (job.includes("BACKEND") || job.includes("LEAD") || job.includes("DATA") || job.includes("ENGINEER"))) return true;
              if (layer === "BACKEND" && (job.includes("BACKEND") || job.includes("API") || job.includes("FULL") || job.includes("ENGINEER"))) return true;
              if (layer === "FRONTEND" && (job.includes("FRONTEND") || job.includes("UI") || job.includes("DESIGN") || job.includes("WEB"))) return true;
              if (layer === "TESTING" && (job.includes("QA") || job.includes("TEST") || job.includes("QUALITY"))) return true;
              if (layer === "DEVOPS" && (job.includes("DEVOPS") || job.includes("CLOUD") || job.includes("ARCHITECT"))) return true;
              return false;

...
import { getEmployeeProductHome } from "@/lib/employees/employee-product-workspace.service";
...
export async function GET(req: NextRequest) {
  try {
    ...
    const data = await getEmployeeProductHome(projectId, employeeId);
    return NextResponse.json({ ok: true, data });
  } catch (err: any) {
    console.error("[api/employee/product/home] error:", err);
    return NextResponse.json({ ok: false, message: err.message || "Failed to load product home." }, { status: 500 });
  }
}

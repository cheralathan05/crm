...
import { getEmployeeMyDayData } from "@/lib/employees/employee-os.service";
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!projectId) {
      return NextResponse.json({ ok: false, message: "No active project found." }, { status: 404 });
    }

    const data = await getEmployeeMyDayData(projectId, employeeId);
    return NextResponse.json({ ok: true, data });
  } catch (err: any) {
    console.error("[api/employee/os/my-day] error:", err);
    return NextResponse.json({ ok: false, message: err.message || "Failed to load My Day data." }, { status: 500 });
  }
}

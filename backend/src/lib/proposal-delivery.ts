...
type ProposalWithClient = Awaited<ReturnType<typeof getProposalWithClient>>;

async function getProposalWithClient(tokenHash: string) {
...
export type ResolvedProposal = {
  proposal: NonNullable<ProposalWithClient>;
  error: "REVOKED" | "EXPIRED" | "REJECTED" | null;
  errorLabel: string | null;
};

export async function resolveProposalByToken(token: string): Promise<ResolvedProposal | null> {
  const tokenHash = hashToken(token.trim());
  const proposal = await getProposalWithClient(tokenHash);
  if (!proposal) return null;
  if (proposal.tokenRevokedAt) {
    return { proposal, error: "REVOKED", errorLabel: proposal.tokenRevokedReason };
  }
  if (proposal.tokenExpiresAt && proposal.tokenExpiresAt < new Date()) {
    return { proposal, error: "EXPIRED", errorLabel: null };
  }
  if (proposal.status === "REJECTED") {
    return { proposal, error: "REJECTED", errorLabel: null };
  }
  return { proposal, error: null, errorLabel: null };
}
...
export async function approveProposal(token: string, input: { clientName?: string }) {
  ...
  // Update real document state to reflect approval
  await syncRealDocuments().catch((e) => console.error("[approval:syncRealDocuments]", e));

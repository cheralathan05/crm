...
export function LeadCopilot({
  ...
  return (
    <div className={cn("flex flex-col", className)}>
      ...
      <div className="mt-3 flex-1 min-h-0">
        <AnimatePresence mode="wait" initial={false}>
          {voiceMode ? (
            ...
            >
              <p className="text-[11px] text-[var(--bos-text-tertiary)]">Ask about this lead.</p>

              {/* Conversation */}
              <div ref={scrollRef} className="mt-3 flex-1 overflow-y-auto pr-0.5 min-h-0 space-y-3">
                    ...
                    <div className="mt-2 flex flex-wrap gap-1.5">
                      {SUGGESTIONS.map((s) => (
                        <button
                          ...
                          onClick={() => void send(s)}
                          disabled={streaming}
                          className="px-2.5 py-1.5 rounded-sm border border-[var(--bos-line)] text-[11px] text-[var(--bos-text-secondary)] hover:border-[var(--bos-border-strong)] hover:text-[var(--bos-text-primary)] hover:bg-[var(--bos-overlay)] transition-colors duration-150 disabled:opacity-40"
                        >
                          {s}
                        </button>
                      ))}
                    </div>
                    <p className="mt-3 text-[11px] text-[var(--bos-text-tertiary)] leading-relaxed">
                ...
                {messages.map((m, i) => {
                  ...
                  return (
                    ...
                    >
                      <div
                        className={cn(
                          "max-w-[92%] text-[12.5px] leading-relaxed whitespace-pre-wrap break-words",
                          isUser ? "rounded-sm bg-[var(--bos-accent)] text-white px-3 py-2" : "text-[var(--bos-text-primary)] px-0.5",
                        )}
                      >
                        {isUser && m.via === "voice" && (
                      ...
                      {actions.length > 0 && (
                        <div className="mt-1.5 flex flex-col gap-1.5 max-w-[92%]">
                          {actions.map((a, ai) => {
                            ...
                            return (
                              <button
                                ...
                                className={cn(
                                  "inline-flex items-center gap-1.5 rounded-sm border px-2.5 py-1.5 text-[11px] font-medium transition-colors duration-150",
                                  done
                ...
                <AnimatePresence>
                  {(streaming || streamText) && (
                    <motion.div
                      ...
                      className="text-[12.5px] leading-relaxed text-[var(--bos-text-primary)] whitespace-pre-wrap break-words"
                    >
      ...
      {/* Context indicator */}
      <div className="mt-3 flex items-center gap-2 text-[10px] text-[var(--bos-text-tertiary)]">
      ...
      {!voiceMode && (
        <button
          type="button"
          onClick={enterVoice}
          className="group mt-2 flex w-full items-center justify-between gap-2 rounded-sm border border-[var(--bos-line)] bg-[var(--bos-surface)]/40 px-3 py-2 transition-all duration-150 hover:border-[var(--bos-accent-ring)] hover:bg-[var(--bos-accent-subtle)]/40"
        >
          <span className="flex items-center gap-2 min-w-0">
            <Mic className="w-3.5 h-3.5 text-[var(--bos-accent)] shrink-0" aria-hidden="true" />
            <span className="text-[11.5px] text-[var(--bos-text-secondary)] group-hover:text-[var(--bos-text-primary)] transition-colors duration-150 truncate">
      ...
      >
        <div className="flex items-center gap-1.5 rounded-sm border border-[var(--bos-line-strong)] bg-[var(--bos-bg)] px-2.5 py-1.5 focus-within:border-[var(--bos-accent)] transition-colors duration-150">
          <input
            value={draft}
            ...
            disabled={streaming}
            className="flex-1 bg-transparent text-[12.5px] outline-none placeholder:text-[var(--bos-text-tertiary)] disabled:opacity-40 min-w-0"
          />
          <button
            type="submit"
            disabled={!draft.trim() || streaming}
            aria-label="Send"
            className="flex items-center justify-center w-6 h-6 rounded-sm text-[var(--bos-text-tertiary)] hover:text-[var(--bos-accent)] hover:bg-[var(--bos-overlay)] disabled:opacity-30 transition-colors duration-150"
          >
